...
$(function(){
	$('#pp').pagination(
	{
		...
		{
			location.href="/front/index/newsListByTag.do?rows=" + pageSize + "&page=" + pageNumber + "&newsTag=${newsTag}";
		}
	});
});	
</script>
<DIV class="lanren box-top">
	<DIV class="search lanren">
		<DIV class="">
			...
			&nbsp;>
			<A href="/front/index/newsListByTag.do?projectKey=${objProject.projectKey }&newsTag=${objNewsTag.lId}" rel="nofollow">${objNewsTag.strTagName}</A>
			&nbsp;>
			列表
		</DIV>
	</DIV>
	...
	<DIV class="list-js">
		...
		<DIV class="nav-sub">
			<s:iterator value="lstProjectMenu" id="projectMenu" >
				<A href="${menuURL}" <s:if test="lId==objSubProjectMenu.lId">class='thisclass'</s:if>><s:property value="strMenuName"/></A>
			</s:iterator>
		</DIV>
		<!-- 详细页右侧导航end -->
