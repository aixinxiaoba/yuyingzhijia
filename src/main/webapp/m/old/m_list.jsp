...
<LINK href="http://www.gmw.cn/favicon.ico?version=3" rel="Shortcut Icon"
	type="image/x-icon">
							...
							<s:iterator value="{1,2,3,4,5}" id="pageNo">
								...
								<s:else>
									<s:if test="#pageNo <= objGPagination.maxPage">
										<A class="pagefontcon" href="/front/m/newsList.do?rows=10&page=${pageNo }&menuID=${menuID}&lProjectMenuID=${objSubProjectMenu.objParentProjectMenu.lId}">${pageNo }</A> 
									</s:if>
								</s:else>
							</s:iterator>
							<s:if test="objGPagination.maxPage != objGPagination.pageNo && && objGPagination.pageNo+2 < objGPagination.maxPage">
								<A class="ptfontcon" href="/front/m/newsList.do?rows=10&page=6&menuID=${menuID}&lProjectMenuID=${objSubProjectMenu.objParentProjectMenu.lId}">›</A>
							</s:if>
						</s:if>
						<s:else>
							<A class="ptfontcon" href="/front/m/newsList.do?rows=10&page=${objGPagination.pageNo-3 }&menuID=${menuID}&lProjectMenuID=${objSubProjectMenu.objParentProjectMenu.lId}">‹</A>
							...
							<s:if test="objGPagination.pageNo+2 >= objGPagination.maxPage">
								<s:iterator value="{objGPagination.maxPage-4,objGPagination.maxPage-3,objGPagination.maxPage-2,objGPagination.maxPage-1,objGPagination.maxPage}" id="pageNos">
									...
									<s:else>
										<A class="pagefontcon" href="/front/m/newsList.do?rows=10&page=${pageNos }&menuID=${menuID}&lProjectMenuID=${objSubProjectMenu.objParentProjectMenu.lId}">${pageNos }</A> 
									</s:else>
								</s:iterator>
							</s:if>
							<s:else>
